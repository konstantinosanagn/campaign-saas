...
OVERVIEW:
The DesignerAgent uses Google's Gemini AI to apply formatting (bold, italic) to email content
  ...
  class DesignAgent
    ...
    def run(writer_output, config: nil)
      ...
      if email_content.blank?
        error_msg = "DesignAgent: No email content provided. Cannot format empty email."
      ...
      unless response.success?
        error_msg = "DesignAgent: Gemini API returned error: #{response.code} - #{response.message}"
        Rails.logger.error("#{error_msg}. Response: #{response.body[0..200]}")
        return {
          email: email_content,
      ...
      if parsed_response["error"]
        error_msg = "DesignAgent: Gemini API error: #{parsed_response["error"]["message"]}"
      ...
      error_msg = "DesignAgent: Failed to parse Gemini API response: #{e.message}"
      Rails.logger.error("#{error_msg}. Response: #{response&.body&.[](0..200)}")
      {
        email: email_content,
        formatted_email: email_content,
      ...
      error_msg = "DesignAgent: Unexpected error: #{e.class} - #{e.message}"
      Rails.logger.error(error_msg)
