...
class Orchestrator
  def initialize
    ...
    @critique_agent = CritiqueAgent.new
  end
  ...
  def run(company_name, recipient: nil, product_info: nil, sender_company: nil)
    ...
    puts "Initial Email generated (#{writer_output[:email].length} characters)"
  ...
  def self.run(company_name, recipient: nil, product_info: nil, sender_company: nil)
    new.run(company_name, recipient: recipient, product_info: product_info, sender_company: sender_company)
  end
end
