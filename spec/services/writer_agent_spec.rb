...
RSpec.describe WriterAgent, type: :service do
  ...
  describe '#initialize' do
    context 'with valid API key' do
      it 'initializes successfully' do
        expect(writer_agent).to be_a(WriterAgent)
      end
  ...
  describe 'HTTParty configuration' do
    it 'includes HTTParty module' do
      expect(WriterAgent.included_modules).to include(HTTParty)
    end

    it 'sets correct base_uri' do
      expect(WriterAgent.base_uri).to eq('https://generativelanguage.googleapis.com/v1beta')
    end
  end
end
