...
class Orchestrator
  def initialize(gemini_api_key:, tavily_api_key:)
    @search_agent = SearchAgent.new(api_key: tavily_api_key)
    @writer_agent = WriterAgent.new(api_key: gemini_api_key)
    @critique_agent = CritiqueAgent.new(api_key: gemini_api_key)
  end
